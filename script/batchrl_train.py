...
raw_trail_name = trail_name
if config.get('use_pav', False):
    pav_suffix = config.get('pav_suffix', 'pav')
    trail_name = raw_trail_name[:-3] + '_' + pav_suffix + '.h5'
dataset_save_path = dataset_dir + '/' + trail_name
dynamics_save_path = output_dir + '/' + 'dynamics' + '_' + raw_trail_name
model_prefix = algo + ('_pav' if config.get('use_pav', False) else '')
model_save_path = output_dir + '/' + model_prefix + '_' + raw_trail_name
scaler = None
