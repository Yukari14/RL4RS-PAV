#!/usr/bin/env bash
# Official modelfree PPO: multi-seed raw vs PAV v2, 10000 epochs (Table 7 scale).
set -euo pipefail
...
LOG="${PILOT_DIR}/ppo_official_10000ep.log"
mkdir -p "$PILOT_DIR"

cd "${rl4rs_benchmark_dir}/script"

echo "=== Official PPO multiseed epochs=${EPOCHS} seeds=${SEEDS[*]} $(date) ===" | tee "$LOG"

for SEED in "${SEEDS[@]}"; do
  echo "" | tee -a "$LOG"
  echo "=== PPO official raw seed=${SEED} epochs=${EPOCHS} $(date) ===" | tee -a "$LOG"
  python -u ppo_pav_pilot.py --seed "${SEED}" --epochs "${EPOCHS}" \
    --batch-size "${BATCH}" --log-every "${LOG_EVERY}" 2>&1 | tee -a "$LOG"

  echo "=== PPO official pav_v2 seed=${SEED} epochs=${EPOCHS} $(date) ===" | tee -a "$LOG"
